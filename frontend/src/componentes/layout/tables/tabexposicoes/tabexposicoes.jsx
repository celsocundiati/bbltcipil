...
function TabExposicoes() {
    ...
    async function handleDelete(id) {
        ...
        try {
            ...
            setToast({
                message: "Erro ao eliminar exposição",
                type: "error",
            });
        }
    }
    ...
    return (
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                        ...
                        {exposicoes.length === 0 ? (
                            ...
                            exposicoes.map(exposicao => (
                                <tr key={exposicao.id} className="hover:bg-black/3">
                                    ...
                                    <td className="flex items-center gap-5 px-5 py-4 text-center text-black/85">
                                        <HiOutlineFolder size={30} className="text-[#F97B17]" />
                                        {exposicao.titulo}
                                    </td>
                                    ...
                                    {podeGerir(user) && (
                                        <td className="px-5 py-4 text-center">
                                            <div className="flex justify-center gap-2">
                                                ...
                                                <button className="cursor-pointer" onClick={() => handleOpenEditModal(exposicao)}>
                                                    <LuFilePen size={25} />
                                                </button>
            ...
            {showModalExposicao && (
                <ModalAddExposicao
                    onClose={() => setShowModalExposicao(false)}
                    showToast={setToast}
                    setExposicoes={setExposicoes}
                />
            )}
